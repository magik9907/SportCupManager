        ...
        public class League
        {
            ...
            public void AutoSchedule(int[] startDate, int spaceBetweenMatches)
            {
                ...
                for (int index = 0; index < rounds.Capacity * (rounds.Capacity / 2 + 1); index++)
                {
                    ...
                    if (i != j)
                        rounds[index / (rounds.Capacity / 2 + 1)].AddMatch(TMatch.Match.CreateMatch(teams[i], teams[j], referees.GetRange((index % (rounds.Capacity / 2 + 1) - 1) * refNumber, refNumber)), referees.GetRange((index % (rounds.Capacity / 2 + 1) - 1) * refNumber, refNumber));
                    else
                        if (teams.Capacity != rounds.Capacity)
                        rounds[index / (rounds.Capacity / 2 + 1)].AddMatch(TMatch.Match.CreateMatch(teams[i], teams[^1], referees.GetRange(referees.Capacity - refNumber, refNumber)), referees.GetRange(referees.Capacity - refNumber, refNumber));
                    i = ++i % rounds.Capacity;
                    j = --j % rounds.Capacity;
                }
            }
